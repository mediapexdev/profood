...
- `android/` est généré et versionné (Capacitor 7, minSdk 23, target/compile 35, portrait, `versionCode 10` / `versionName 2.0.0`). Outillage sans Android Studio : SDK dans `/opt/homebrew/share/android-commandlinetools` (`brew install --cask android-commandlinetools`), **JDK 21 obligatoire** (`brew install openjdk@21`, `JAVA_HOME=/opt/homebrew/opt/openjdk@21`), `android/local.properties` (ignoré) pointe `sdk.dir`.
