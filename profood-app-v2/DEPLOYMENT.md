...
- `ios/` est généré et versionné (Capacitor 7, cible iOS 14, iPhone verrouillé en portrait, version marketing `2.0.0`).
- `android/` est généré et versionné (Capacitor 7, minSdk 23, target/compile 35, portrait, `versionCode 10` / `versionName 2.0.0`). **`applicationId` Android = `sn.profood.app`**, et depuis le 2026-09-13 le bundle iOS aussi (`com.profoodapp.app` est réservé par d'autres comptes sur Google Play comme chez Apple ; App ID `sn.profood.app` enregistré sur le Team 556TCV8AUY, fiche App Store Connect 6811567602 créée) ; le namespace Java reste `com.profoodapp.app`. Outillage sans Android Studio : SDK dans `/opt/homebrew/share/android-commandlinetools` (`brew install --cask android-commandlinetools`), **JDK 21 obligatoire** (`brew install openjdk@21`, `JAVA_HOME=/opt/homebrew/opt/openjdk@21`), `android/local.properties` (ignoré) pointe `sdk.dir`.
- **Signature Android** : clé d'upload `~/Keys/profood/profood-upload.jks` (alias `profood`, validité 10 000 jours) + `~/Keys/profood/keystore.properties` (mots de passe), copié en `android/keystore.properties` (ignoré par git, lu par `app/build.gradle`). ⚠️ Sauvegarder ce dossier hors du Mac : sans la clé, aucune mise à jour Play possible. Build : `cd android && JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew bundleRelease` → `app/build/outputs/bundle/release/app-release.aab`. À chaque nouvelle version, incrémenter `versionCode`.
