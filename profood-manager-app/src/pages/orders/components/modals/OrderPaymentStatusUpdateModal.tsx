...
import { OrderPaymentStatus, OrderProps } from "../../../../types";
import { useUserInfosContext } from "../../../account/components/contexts/UserInfosProvider";

import './OrderPaymentStatusUpdateModal.css';

/**
 * 
 */
export interface OrderPaymentStatusUpdateModalProps {
    show: boolean;
 ...
 * @returns 
 */
const OrderPaymentStatusUpdateModal: React.FC<OrderPaymentStatusUpdateModalProps> = ({order, show, setShow, toggle, onClosed}: OrderPaymentStatusUpdateModalProps) => {
    /**
     * 
     */
     ...
     * 
     */
     const { orderPaymentStatuses, fetchOrders } = useDataContext();

    /**
     * 
     */
    const [status, setStatus] = useState<OrderPaymentStatus|undefined>(undefined);
     ...
     * @param _status 
     */
    const changeStatus = (_status?: OrderPaymentStatus) => {
        setStatus(_status);
    ...
    return (
        <Modal
            id='orderPaymentStatusUpdateModal'
            isOpen={show}
            toggle={toggle}
            onClosed={onClosed}
            size='md'
        ...
        >
            <ModalHeader className='flex-center border-0 py-5 px-sm-4'>
                <span className="fs-6">{t('Mise à jour du statut de paiement')}</span>
            </ModalHeader>
...
export default OrderPaymentStatusUpdateModal;
