...
import './CustomerProfileDetailsEditingFormView.css';
 ...
 * @returns 
 */
const CustomerProfileDetailsEditingFormView: React.FC<CustomerProps> = (customer: CustomerProps) => {
    /**
     * 
     */
    ...
    useEffect(() => {
        if(reset){
            changeFirstName(customer.user.first_name);
            changeLastName(customer.user.last_name);
            changePhoneNumber(customer.user.phone_number);
            changeEmail(customer.user.email);
            setPreview(customer.user.avatar);
    ...
    const update = () => {
        ...
        api.post('/update-customer-profile-details', data,
            {
                headers: {
            ...
            if(res.status === 200 && res.data.message){
                fetchCustomers(true, 3700);
                showToast(t(res.data.message), 'success', {autoClose: 2000});
                setTimeout(() => {
                    goTo(`/clients/vue/${customer.id}`);
                }, 3000);
            }
    ...
    return (
        <Card
            id="customerProfileDetailsEditingFormView"
            className="border-0"
        >
            <CardHeader className="bg-transparent py-5 px-6">
                                    ...
                                    >
                                        ...
                                        >
                                            <Button
                                                ...
                                                onClick={() => {
                                                    setAvatar(undefined);
                                                    setPreview(selectedFile ? customer.user.avatar : undefined);
...
export default CustomerProfileDetailsEditingFormView;
