...
//var_dump($rawResponse);
        ...
        if (array_key_exists('token', $jsonResponse)) {
            ...
            Log::info('PayTech payment request successful - token received', [
                'item_name' => $params['item_name'],
                'ref_command' => $params['ref_command'],
                'token' => substr($jsonResponse['token'], 0, 10) . '...', // Log only first 10 chars of token for security
                'action' => 'PayTech::send'
            ]);
            ...
            // Log payment request failure with errors
            Log::error('PayTech payment request failed with errors', [
                'item_name' => $params['item_name'],
                'ref_command' => $params['ref_command'],
            ...
            // Log internal payment request error
            Log::error('PayTech payment request failed - internal error', [
                'item_name' => $params['item_name'],
                'ref_command' => $params['ref_command'],
                'raw_response' => substr($rawResponse, 0, 500), // Log first 500 chars of raw response
                'action' => 'PayTech::send'
            ]);
    ...
    private static function post($url, $data = [], $header = [])
    {
        ...
        // Only disable in development if absolutely necessary via env variable
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, !env('PAYTECH_DISABLE_SSL_VERIFY', false));
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, !env('PAYTECH_DISABLE_SSL_VERIFY', false) ? 2 : 0);
